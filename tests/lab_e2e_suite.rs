...
use kerosene_vault::application::{
    ...
    GetAllowlist, LedgerPort, MutableOnlineCount, ProposeEpochAdvance, ProposeRelease,
    RebuildRelease, ReleaseStorePort, SignMessage, VoteEpochAdvance,
};
use kerosene_vault::domain::{
    run_dkg, BucketKind, Constitution, ContentHash, NodeId, ReleasePolicy, SettlementIntent,
};
...
fn suite_happy_intent_gate_then_frost_sign() {
    ...
    let state = Arc::new(ThresholdVaultState::new(
        group,
        shares[0].clone(),
        shares,
    ));
    ...
    let sig = sign
        .run_lab_quorum_sign("sess-happy", "msg-hash-happy")
        .unwrap();
...
fn suite_fail_stop_when_online_below_t() {
    ...
    let sign = SignMessage::new(state, online);
    let err = sign
        .run_lab_quorum_sign("sess-fail", "mh")
        .unwrap_err()
        .to_string();
...
fn suite_release_clean_and_tampered() {
    ...
    propose
        .execute("rel-ok", b"src-clean", council())
        .unwrap();
    ...
    for id in ids.iter().take(2) {
        CosignRelease::new(release.clone(), ledger.clone(), clock.clone(), id.clone())
            .execute("rel-ok")
            .unwrap();
    }
    let entry = ActivateRelease::new(release.clone(), ledger.clone(), clock.clone())
        .execute("rel-ok")
        .unwrap();
    GetAllowlist::new(release.clone())
        .require_hb(&entry.hb)
        .unwrap();
    ...
    propose
        .execute_with_hashes("rel-bad", hs, evil, council())
        .unwrap();
    let err = rebuild
        .execute("rel-bad", &ids[0])
        .unwrap_err()
        .to_string();
...
fn suite_governance_epoch_still_advances() {
    ...
    ProposeEpochAdvance::new(ledger.clone(), ids[0].clone())
        .execute("e2e-epoch")
        .unwrap();
    VoteEpochAdvance::new(ledger.clone(), ids[1].clone())
        .execute("e2e-epoch")
        .unwrap();
    let p = VoteEpochAdvance::new(ledger.clone(), ids[2].clone())
        .execute("e2e-epoch")
        .unwrap();
    assert!(p.closed);
