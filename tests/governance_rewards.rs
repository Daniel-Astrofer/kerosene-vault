...
use kerosene_vault::adapters::{
    FrostShareSlot, FrostShareState, FrostTrShareSlot, InMemoryEconomy, InMemoryLedger,
    InMemoryReleaseMesh, PolicyReshareHook,
};
use kerosene_vault::adapters::DistributedDkgAdapter;
use kerosene_vault::application::{
    AccrueGovernanceWork, ActivateRelease, BlobStorePort, ClockPort, CosignRelease,
    EconomyPort, GetEconomyStatus, LedgerPort, ProposeRelease, RebuildRelease, ReleaseStorePort,
...
fn day_rotation_and_reshare_accrue_governance_rewards() {
    ...
    for (i, id) in ids.iter().enumerate() {
        upsert
            .execute(eligible_op(id.as_str(), &format!("bc1q-vault-{i}")))
            .unwrap();
    }

    let gov = Arc::new(AccrueGovernanceWork::new(
        economy.clone(),
        ledger.clone(),
        ids[0].clone(),
        gov_cfg(),
    ...
    assert!(kinds
        .iter()
        .filter(|k| **k == LedgerEventKind::GovernanceRewardAccrued)
        .count()
        >= 2);
...
fn release_cosign_and_activate_accrue_governance_rewards() {
    ...
    for (i, id) in ids.iter().enumerate() {
        upsert
            .execute(eligible_op(id.as_str(), &format!("bc1q-rel-{i}")))
            .unwrap();
    }
    let gov = Arc::new(AccrueGovernanceWork::new(
        economy.clone(),
    ...
    ActivateRelease::new(release_port, ledger.clone(), clock)
        .with_governance(gov.clone())
        .execute("rel-gov")
        .unwrap();
    ...
    let gov_events = ledger
        .entries()
        .unwrap()
        .into_iter()
        .filter(|e| e.kind == LedgerEventKind::GovernanceRewardAccrued)
        .count();
    assert_eq!(gov_events, 3); // 2 cosign + 1 activate

    // Disabled config accrues nothing further.
    let disabled = AccrueGovernanceWork::new(
        economy.clone(),
        ledger,
        ids[0].clone(),
        GovernanceRewardConfig::disabled(),
    );
