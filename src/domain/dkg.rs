...
pub fn run_dkg(
    ...
    t: usize,
    entropy: &[u8],
) -> Result<(GroupKey, Vec<KeyShare>), DomainError> {
    ...
    for (i, node) in active_set.iter().enumerate() {
        ...
        commitments.push(format!(
            ...
            crate::domain::attestation::Measurement::from_bytes(
                format!("commit:{}", coeffs[0]).as_bytes(),
            )
            .as_hex()
        ));
    ...
    for (j, node) in active_set.iter().enumerate() {
        ...
        shares.push(KeyShare {
            index: ShareIndex::new((j as u8) + 1)?,
            value,
            node_id: node.clone(),
        });
    }

    let commitment = crate::domain::attestation::Measurement::from_bytes(
