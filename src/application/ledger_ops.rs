...
impl ProposeEpochAdvance {
    ...
    pub fn execute(&self, proposal_id: &str) -> Result<EpochAdvanceProposal, DomainError> {
        ...
        if !epoch.contains(&self.local_node) {
            return Err(DomainError::UnauthorizedWriter(
...
impl VoteEpochAdvance {
    ...
    pub fn execute(&self, proposal_id: &str) -> Result<EpochAdvanceProposal, DomainError> {
        ...
        if proposal.from_epoch != epoch.number {
            return Err(DomainError::EpochMismatch {
        ...
        if proposal.constitution_hash != constitution.hash {
            return Err(DomainError::LedgerConflict(
                "proposal constitution hash diverges from active".into(),
            ));
        }
    ...
    fn advance_epoch(&self, proposal: &EpochAdvanceProposal) -> Result<(), DomainError> {
        ...
        let entry = LedgerEntry::chain(
            ...
            &format!(
                r#"{{"to_epoch":{},"constitution_hash":"{}"}}"#,
                proposal.to_epoch, proposal.constitution_hash
            ),
            self.local_node.clone(),
            &prev,
        );
...
impl LedgerSnapshot {
    pub fn to_json(&self) -> String {
        let entries = self
            ...
            .join(",");
        format!(
            r#"{{"constitution":{},"epoch":{},"entries":[{}]}}"#,
            self.constitution_json, self.epoch_json, entries
        )
    }
}
