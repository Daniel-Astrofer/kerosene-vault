...
use crate::domain::{
    Constitution, DomainError, Epoch, EpochAdvanceProposal, LedgerEntry, LedgerEventKind, NodeId,
};
        ...
        if let Some(prev) = guard.entries.last() {
            ...
            if entry.prev_hash != prev.entry_hash {
                return Err(DomainError::LedgerConflict(
                    "prev_hash does not match head".into(),
                ));
            }
        } else if entry.index != 0 {
            return Err(DomainError::LedgerConflict(
                "first entry must be index 0".into(),
            ));
        }
