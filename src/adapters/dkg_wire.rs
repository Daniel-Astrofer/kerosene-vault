...
//! round packages over HTTP (`X-Vault-Token`). After part3 the vault persists
//! **only its own** key package. ToB 2024: `min_signers` must match constitution.
//!
...
impl WireDkgHub {
    ...
    /// Start local session: run part1, store own package, return wire message for fan-out.
    ...
    pub fn ingest_round1(&self, msg: Round1WireMessage) -> Result<WireDkgStatus, DomainError> {
        ...
        if msg.max_signers != session.max_signers || msg.min_signers != session.min_signers {
            return Err(DomainError::ThresholdError(
                "round1 package constitution params mismatch".into(),
            ));
        }
    ...
    /// Fan-out round1 package to configured peers (best-effort HTTP).
    pub async fn fanout_round1(&self, msg: &Round1WireMessage) -> Result<(), DomainError> {
        for (peer_id, addr) in &self.peer_addrs {
            ...
            let url = format!("http://{addr}/v1/dkg/round1");
            let res = self
                .http
                .post(&url)
    ...
    pub async fn fanout_round2(&self, messages: &[Round2WireMessage]) -> Result<(), DomainError> {
        for msg in messages {
            ...
            let url = format!("http://{addr}/v1/dkg/round2");
            let res = self
                .http
                .post(&url)
                .header("X-Vault-Token", &self.token)
...
mod tests {
    ...
    fn wire_dkg_three_party_message_exchange_no_dealer() {
        ...
        // Each hub ingests others' round1
