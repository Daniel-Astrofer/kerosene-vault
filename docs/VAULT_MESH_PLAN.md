...
| **Tor private mesh (ceremony)** | **MVP landed** | `VAULT_TRANSPORT=tor` + `VAULT_SOCKS_PROXY` (socks5h) + onion `VAULT_SEED_PEERS`; Tor timeouts/retries+jitter on wire DKG / anti-nonce; compose `vault-mesh-tor.compose.yaml` + `scripts/lab_dkg_wire_tor.sh`; production hygiene refuses clearnet transport / `VAULT_CLEARNET_PUBLISH`; docs `backend/kerosene-vault/docs/CEREMONY_TOR.md`. **Gaps:** mTLS+.onion SAN/SPIFFE verify; deploy.sh still clearnet lab |
