...
| **CHANNELS → LND inject** | landed (lifecycle couple) | Soft-reserve CHANNELS (`/v1/intent/reserve`) → LND `openChannel` → commit/release; go-live `kfe.channel.require-channels-mesh-inject=true` + `capacity.auto-open=false`. Residual: LND wallet still supplies funding UTXO (no CHANNELS→LND on-chain inject PSBT); pending-open not in `listChannels`; open-ok/commit-fail window |
