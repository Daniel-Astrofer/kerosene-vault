...
| **CHANNELS → LND inject** | landed (atomicity harden) | Soft-reserve CHANNELS → LND address bind → `openChannel` → commit; pending-channels refuse; durable phase + stable Intent id (resume); commit-retry reconciler + orphan reserve release. Residual: **no on-chain CHANNELS→LND PSBT** (shared Taproot USERS-only / per-bucket key TBD) — LND wallet UTXOs still pay the funding tx after ledger bind + balance check |
